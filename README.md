...
- **Current runtime readiness: `runtime_enablement_candidate_with_limitations`** — strict enforcement is incomplete: non-allow decisions do not yet block execution in development/safe modes, `/role revoke` is not yet governed, and capability gate checks per action are not yet enforced. All validators, docs, and tests truthfully reflect these limitations.
