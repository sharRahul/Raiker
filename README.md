...
| Phase 3 local rich workspace/extensibility foundations | **Foundation only; Phase 3 is not complete** | Read-only workspace inspection/view surfaces, equal client contract parity, plugin manifest/registration planning, capability gates, graph/codemap dry-run planning, semantic memory review governance, approval-preview UX/contracts, Slice G metadata-only storage lifecycle preparation, Slice H metadata-only retention/cleanup-preview/approval-handoff planning, and Slice I metadata-only lifecycle evidence/export/policy-simulation surfaces are present. Runtime plugin execution, graph indexing, semantic/vector memory writes, embeddings, cleanup execution, approval relay, and durable approval-preview persistence remain disabled. |
