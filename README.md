...
- Real executors are exactly the capabilities in `REAL_EXECUTOR_CAPABILITIES`; those integrated gates default `enabled_runtime` and are governed per action (decision mode default `ask`). This `enabled_runtime` default is the **static/global** posture (the single-user CLI runtime). The **web dashboard is per-principal and fails closed**: each account owner sees every gate `disabled` until they turn it on for their own workspace, and reaching `enabled_runtime` additionally requires an active runtime-enablement mode (`local_single_user_runtime`/`multi_user_local_runtime`) — the default **Development preview** keeps everything off. This is why a fresh `raiker-web` workspace shows all gates disabled. See [`docs/RUNTIME_EXECUTORS_SPEC.md`](docs/RUNTIME_EXECUTORS_SPEC.md).
...
- **Capability registry** is expanded to 53 capabilities across all domain runtimes. Integrated capabilities (those with a real executor, `REAL_EXECUTOR_CAPABILITIES`) default to `enabled_runtime` **in the static/global (single-user CLI) posture** and are governed per action (decision mode, default `ask`); capabilities that are not integrated yet default to `disabled` and fail closed. Under the **web dashboard's per-principal controls every gate reads `disabled` until the account owner turns it on**, and only a runtime-enablement mode (not Development preview) permits the `enabled_runtime` transition. The `ALL_CAPABILITIES` and `RUNTIME_DOMAIN_CAPABILITIES` sets are defined in `raiker/phase_gates.py`.
...
- The dedicated current security architecture, trust-boundary model (including the web dashboard), and deferred-control gates are documented in [`SECURITY.md`](SECURITY.md) and [`docs/SECURITY_ARCHITECTURE.md`](docs/SECURITY_ARCHITECTURE.md). The owner's normative security policy (User-Centric Zero Trust, requirements ZT-1 … ZT-12) is [`docs/USER_CENTRIC_ZERO_TRUST_POLICY.md`](docs/USER_CENTRIC_ZERO_TRUST_POLICY.md).
