...
// `rows` are spreadsheet cell values, and `pdf_url` is a same-origin authorized
// path fetched with the session bearer token — never an external URL.
export interface AttachmentPreview {
  attachment_id: string;
  session_id: string;
  filename: string;
  media_type: string;
  kind: "text" | "markdown" | "table" | "pdf" | "unavailable";
  byte_size: number;
  text: string;
  rows: string[][];
  truncated: boolean;
