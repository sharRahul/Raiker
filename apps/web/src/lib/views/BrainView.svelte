...
<style>
  ...
  .empty-copy { position:absolute; z-index:4; left:50%; top:20px; transform:translateX(-50%); display:grid; gap:4px; width:min(520px, 80%); text-align:center; pointer-events:none; } .empty-copy strong { font-size:.92rem; } .empty-copy span { color:var(--text-2); font-size:.75rem; line-height:1.45; }
  ...
  .summary-pill { left:16px; top:16px; gap:8px; border-radius:20px; padding:7px 11px; font:inherit; font-size:.7rem; cursor:pointer; } .summary-pill i { width:3px; height:3px; border-radius:50%; background:var(--text-3); }
  .summary-popover { position:absolute; z-index:8; left:16px; top:56px; width:230px; padding:14px; border:1px solid var(--border-strong); border-radius:10px; background:color-mix(in srgb, var(--raised) 96%, transparent); box-shadow:var(--shadow-2); } .summary-popover h3 { margin:0 0 10px; font-size:.78rem; } .summary-popover p { display:flex; justify-content:space-between; margin:6px 0; color:var(--text-2); font-size:.72rem; } .summary-popover p b { color:var(--text-1); } .summary-popover small { display:flex; gap:5px; align-items:center; margin-top:12px; padding-top:10px; border-top:1px solid var(--border); color:var(--accent); font-size:.65rem; }
  .viewport-controls { right:16px; bottom:16px; border-radius:8px; overflow:hidden; } .viewport-controls button { height:32px; min-width:34px; border:0; border-right:1px solid var(--border); background:transparent; color:var(--text-2); cursor:pointer; } .viewport-controls button:first-child { padding:0 11px; font-size:.68rem; } .viewport-controls span { min-width:46px; text-align:center; font-size:.65rem; }
  .graph-meta { left:16px; bottom:16px; gap:7px; padding:7px 10px; border-radius:7px; font-size:.65rem; } .graph-meta button { border:0; background:transparent; color:var(--accent); font:inherit; cursor:pointer; } .live-dot { width:6px; height:6px; border-radius:50%; background:var(--success); box-shadow:0 0 7px var(--success); }
  .depth-control { left:50%; bottom:16px; transform:translateX(-50%); gap:10px; padding:8px 12px; border-radius:8px; font-size:.68rem; } .depth-control input { width:130px; accent-color:var(--accent); }
  ...
  .motion-options { display:grid; grid-template-columns:repeat(3, 1fr); gap:4px; } .motion-options label { display:flex; align-items:center; gap:3px; color:var(--text-2); font-size:.62rem; }
  .inspector { padding:18px; width:280px; bottom:14px; } .inspector .close,.source-modal .close { position:absolute; right:12px; top:9px; } .record-kicker { display:flex; align-items:center; gap:7px; color:var(--text-3); text-transform:uppercase; letter-spacing:.11em; font-size:.6rem; } .inspector h3 { margin:10px 24px 4px 0; font-size:1.05rem; } .status-line { display:flex; gap:8px; color:var(--text-3); font-size:.66rem; } .status-line span { padding:3px 6px; border:1px solid var(--border); border-radius:4px; } .inspector > p { color:var(--text-2); font-size:.73rem; line-height:1.55; } .inspector h4 { margin:20px 0 7px; color:var(--text-3); text-transform:uppercase; letter-spacing:.1em; font-size:.62rem; }
  .relationship { display:grid; width:100%; gap:2px; padding:8px 0; border:0; border-bottom:1px solid var(--border); background:transparent; text-align:left; cursor:pointer; } .relationship span { color:var(--text-3); font-size:.6rem; } .relationship b { color:var(--text-1); font-size:.72rem; } .inspector-actions { display:grid; gap:7px; margin-top:18px; } .inspector-actions button { border:1px solid var(--border); border-radius:6px; padding:7px; background:var(--sunken); color:var(--text-2); cursor:pointer; }
  .relationship-row { display:grid; grid-template-columns:1fr auto; align-items:center; border-bottom:1px solid var(--border); } .relationship-row .relationship { border:0; } .relationship-row small { grid-column:1/-1; color:var(--text-3); font-size:.58rem; overflow-wrap:anywhere; } .reject-link { border:0; background:transparent; color:var(--danger); font-size:.62rem; cursor:pointer; }
  .source-modal::backdrop { background:var(--overlay); backdrop-filter:blur(5px); } .source-modal { position:relative; width:min(480px, calc(100vw - 32px)); max-height:80vh; overflow:auto; padding:24px; border:1px solid var(--border-strong); border-radius:13px; background:var(--raised); color:var(--text-1); box-shadow:var(--shadow-2); } .source-modal h2 { margin:6px 0; font-size:1.15rem; } .source-modal > p { color:var(--text-2); font-size:.75rem; line-height:1.5; } .source-modal form label { display:grid; gap:6px; color:var(--text-2); font-size:.7rem; } .source-modal form input { border:1px solid var(--border); border-radius:6px; padding:10px; background:var(--sunken); color:var(--text-1); } .primary { width:100%; margin-top:12px; border:0; border-radius:6px; padding:9px; background:var(--accent); color:var(--text-inverse); cursor:pointer; } .error { color:var(--danger) !important; } .current-source { display:flex; justify-content:space-between; padding:7px 0; border-bottom:1px solid var(--border); color:var(--text-2); font-size:.7rem; } .current-source button { border:0; background:transparent; color:var(--danger); cursor:pointer; }
  ...
  .source-browser button.scope-root { display:grid; gap:2px; padding:9px; } .source-browser button.scope-root b { font-size:.76rem; } .source-browser button.scope-root small { padding:0; color:var(--text-3); font-size:.65rem; line-height:1.45; } .source-browser button.scope-root span { width:auto; }
  .source-browser button.disabled { cursor:default; opacity:.72; }
  .source-browser button.revoke { justify-content:flex-end; padding:5px 9px 9px; color:var(--danger); font-size:.65rem; }
  .from-computer { margin-top:16px; padding-top:14px; border-top:1px solid var(--border); } .from-computer h3 { margin:0 0 8px; font-size:.82rem; } .from-computer form button { width:100%; margin-top:8px; border:1px solid var(--border); border-radius:6px; padding:8px; background:transparent; color:var(--text-2); cursor:pointer; } .upload { margin-top:14px; display:grid; gap:8px; color:var(--text-2); font-size:.7rem; } .upload input[type="file"] { color:var(--text-2); font-size:.68rem; } .consent { display:flex !important; align-items:flex-start; gap:8px; line-height:1.45; }
