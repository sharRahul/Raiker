<script lang="ts">
  ...
  const agents = $derived(graphNodes.filter((node) => node.node_type === "agent"));
...
{:else}
  ...
  <div class="brain-layout">
    ...
    <aside class="work-panel">
      ...
      <section class="card work-action">
        <h2>Work in Action</h2>
        {#if agents.length || tasks.length || waiting.length}
          {#each agents as agent (agent.node_id)}
            <div class="work-row"><span class="mini-agent" aria-hidden="true"></span><div><b>{agent.label}</b><small>{agent.status}{agent.detail ? ` · ${agent.detail}` : ""}</small></div></div>
          {/each}
          {#each tasks.filter((task) => ACTIVE.has(task.status)) as task (task.node_id)}
            <div class="work-row"><span class="task-mark" aria-hidden="true"></span><div><b>{task.label}</b><small>{task.status}{task.progress_percent !== null ? ` · ${task.progress_percent}%` : ""}</small></div></div>
          {/each}
          {#each waiting as schedule (schedule.node_id)}
            <div class="work-row"><span class="wait-mark" aria-hidden="true"></span><div><b>Scheduled work</b><small>waiting · {schedule.detail}</small></div></div>
          {/each}
        {:else}
          <p>No active or scheduled work is stored right now.</p>
        {/if}
      </section>
    </aside>
  </div>
...
<style>
  ...
  .flow h2,.graph-heading h2,.inspector h2,.work-action h2 { font-size:var(--text-base); margin:0; }
  .flow p,.graph-heading p,.inspector p,.work-action p { color:var(--text-muted); font-size:var(--text-sm); margin:4px 0 0; }
  ...
  .brain-layout { display:grid; grid-template-columns:minmax(0, 1fr) minmax(260px, 320px); gap:var(--space-4); }
