...
describe("ChatView streaming transcript", () => {
  ...
  it("copies a response without offering transcript exports", async () => {
    ...
    expect(screen.queryByRole("button", { name: "Export as Markdown" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Print / Save as PDF" })).not.toBeInTheDocument();
  });

  it("opens a file generated by the completed turn in the right-hand inspector", async () => {
    stubFetch({
      ...MODELS_ROUTE,
      "GET /api/sessions/sess_1/attachments": {
        session_id: "sess_1",
        files: [{
          attachment_id: "att_generated",
          turn_id: "turn_1",
          kind: "document",
          filename: "draft.md",
      ...
      "GET /api/sessions/sess_1/attachments/att_generated/preview": {
        attachment_id: "att_generated",
        session_id: "sess_1",
        filename: "draft.md",
        media_type: "text/markdown",
    ...
    streamPromptMock.mockImplementation(
      async (_body: unknown, onEvent: (ev: StreamEvent) => void) => {
        onEvent({ kind: "final", text: "", event_type: "", payload: {}, response: finalResponse("Created it.") } as StreamEvent);
      },
    );

    render(ChatView);
    await fireEvent.input(screen.getByRole("textbox", { name: /prompt/i }), { target: { value: "Create a draft" } });
    await fireEvent.keyDown(screen.getByRole("textbox", { name: /prompt/i }), { key: "Enter" });

    await fireEvent.click(await screen.findByRole("button", { name: /draft\.md/i }));
