...
<div class="header"><p class="page-lead">Connect services and use their approved actions in chat.</p><button class="btn" onclick={() => (showManifest = true)}><Icon name="file" size={15}/> Import manifest</button></div>
...
<div class="chip-row categories" role="group" aria-label="Filter connectors by category">{#each categories as item (item)}<button type="button" class="chip" aria-pressed={category === item} onclick={() => (category = item)}>{item}</button>{/each}</div>
