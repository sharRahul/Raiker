<script lang="ts">
  ...
  const categories = $derived([
    "All",
    ...new Set((view?.connectors ?? []).map((c) => c.category)),
  ]);
  const filtered = $derived(
    (view?.connectors ?? []).filter(
      (c) =>
  ...
  async function load() {
    ...
    try {
      ...
      if (selected)
        selected =
          view.connectors.find(
            (c) => c.connector_id === selected?.connector_id,
          ) ?? null;
    } catch (e) {
      error = reason(e);
    }
  ...
  async function install(c: StoreConnector) {
    ...
    error = null;
    try {
      await api.installConnector(c.connector_id);
      await load();
    } catch (e) {
      error = reason(e);
    } finally {
  ...
  async function authenticate(c: StoreConnector) {
    ...
    error = null;
    try {
      await api.setConnectorCredentials(
        c.connector_id,
        c.auth_type === "oauth2"
          ? { access_token: secret }
          : { api_key: secret },
  ...
  async function toggle(c: StoreConnector) {
    ...
    error = null;
    try {
      await api.setConnectorEnabled(c.connector_id, !c.enabled);
      await load();
    } catch (e) {
      error = reason(e);
    } finally {
  ...
  async function uninstall(c: StoreConnector) {
    ...
    error = null;
    try {
      await api.uninstallConnector(c.connector_id);
      selected = null;
      await load();
    } catch (e) {
      error = reason(e);
  ...
  async function connectViaMcp() {
    if (
      !mcpConnector ||
      !mcpName.trim() ||
      (mcpTransport === "remote" && !endpointUrl.trim())
    )
      return;
    busy = true;
    error = null;
  ...
  async function registerManifest() {
    ...
    try {
      ...
      const result = await api.registerConnectorManifest(
        manifestConnectorId,
        manifest,
      );
      manifestResult = `${result.operations?.length ?? 0} operations registered`;
    } catch (e) {
      manifestResult = reason(e);
    }
  }
  ...
  function lifecycle(c: StoreConnector): string {
    ...
    if (c.auth_status === "reauth_required")
      return "Requires re-authentication";
    if (!c.enabled)
      return c.auth_status === "connected"
        ? "Disabled"
...
{#if !view}<PageState state="loading" title="Loading connectors…" />{:else}<div
    class="grid"
  >
    {#each filtered as c (c.connector_id)}<article>
        <button
          class="summary"
          aria-label={`Open ${c.display_name}`}
          onclick={() => (selected = c)}
          ><span class="logo">{c.display_name[0]}</span><span
            ><span class="title"
              >{c.display_name}<small>{c.category}</small></span
            ><span class="description">{c.description}</span></span
          ></button
        >
        <footer>
          <span
            class:connected={lifecycle(c) === "Connected"}
            class:warning={c.auth_status === "reauth_required"}
            class="status"><i></i>{lifecycle(c)}</span
          ><span class="footer-actions"
            ><button
              class="btn btn-sm"
              onclick={() => openMcp(c)}
              disabled={busy}
              aria-label={`Connect ${c.display_name} via MCP`}
              >Connect via MCP</button
            ><button
              class="btn btn-sm"
              class:btn-primary={!c.installed}
              onclick={() => (c.installed ? (selected = c) : install(c))}
              disabled={busy}>{c.installed ? "Manage" : "Install"}</button
            ></span
          >
        </footer>
...
{#if selected}<div
  ...
  >
    ...
    >
      ...
      <div class="state">
        <strong>{lifecycle(selected)}</strong><span
          >{selected.enabled
            ? "Available in the current session"
            : "Not available in the current session"}</span
        >
      </div>
      {#if !selected.installed}<p>
          Install this connector to configure authentication and register its
          manifest.
        </p>
        <button
        ...
        >{:else}<section>
          ...
          <p>
            {selected.auth_type === "oauth2"
              ? "OAuth 2.0 access and refresh credentials are encrypted for this profile."
              : "The API key is encrypted for this profile."}
          </p>
          {#if selected.auth_status !== "connected"}<label
              class="field-label"
              for="credential"
              >{selected.auth_type === "oauth2"
                ? "OAuth access token"
        ...
        <section>
          ...
          {#if selected.operations.length > 0}
            <ul class="operations">
              {#each selected.operations as operation (operation.operation_id)}
                <li>
                  <span class:write={operation.requires_confirmation} class="method">
                    {operation.method}
                  </span>
                  <span class="operation-copy">
                    <strong>{operation.description || operation.operation_id}</strong>
                    <small>{operation.path}</small>
                  </span>
                  {#if operation.requires_confirmation}<span class="pill">Preview + approval</span>{/if}
                </li>
              {/each}
            </ul>
          {:else}
            <p>No manifest operations registered yet.</p>
          {/if}
        </section>
        <section>
          <h3>Session access</h3>
          <p>
            Reads follow policy. Writes show the exact redacted request body and stop for approval
            before sending.
          </p>
          <button
            class="btn"
            class:btn-danger={selected.enabled}
            onclick={() => toggle(selected!)}
            disabled={busy || selected.auth_status !== "connected"}
            >{selected.enabled
              ? "Disable for session"
              : "Enable for session"}</button
          >
        </section>
        <div class="danger-zone">
          <button
            ...
            disabled={busy}>Uninstall and remove credentials</button
          >
        </div>{/if}
    </div>
  </div>{/if}
...
{#if mcpConnector}<div
    class="overlay"
    role="presentation"
    onclick={(e) => e.target === e.currentTarget && (mcpConnector = null)}
  >
    <div
      class="dialog"
      role="dialog"
    ...
    >
      ...
      <p>
        Add a monitored local starter or a remote MCP server. Raiker stores only
        a token environment-variable name, never its value.
      </p>
      <label class="field-label" for="mcp-name">Connection name</label><input
        id="mcp-name"
        class="input wide"
        bind:value={mcpName}
      /><label
        ><input
          type="radio"
          name="mcp-transport"
          bind:group={mcpTransport}
          value="local"
        /> Local starter server</label
      ><label
        ><input
          type="radio"
          name="mcp-transport"
      ...
      >{#if mcpTransport === "remote"}<label
          class="field-label"
          for="mcp-endpoint">MCP endpoint URL</label
        ><input
          id="mcp-endpoint"
          class="input wide"
          type="url"
        ...
        ><input
          ...
          autocomplete="off"
        />{/if}{#if mcpResult}<div class="notice notice-ok">
          {mcpResult}
        </div>{/if}
      <div class="actions">
        <button
          class="btn btn-primary"
          onclick={connectViaMcp}
          disabled={busy ||
            !mcpName.trim() ||
            (mcpTransport === "remote" && !endpointUrl.trim())}
          >{busy ? "Connecting..." : "Connect via MCP"}</button
        >
      </div>
    </div>
  </div>{/if}
...
{#if showManifest}<div
    class="overlay"
    role="presentation"
    onclick={(e) => e.target === e.currentTarget && (showManifest = false)}
  >
    <div
      class="dialog"
      role="dialog"
    ...
    >
      ...
      <p>
        OpenAPI 2/3 and ai-plugin.json metadata are validated and compiled
        server-side.
      </p>
      <label class="field-label" for="connector-id">Connector</label><select
        id="connector-id"
        class="select wide"
        bind:value={manifestConnectorId}
        ><option value="">Select connector</option
        >{#each view?.connectors ?? [] as c (c.connector_id)}<option
            value={c.connector_id}>{c.display_name}</option
          >{/each}</select
      ><label class="field-label" for="manifest-json">Manifest JSON</label
      ><textarea
        id="manifest-json"
        class="textarea manifest"
        bind:value={manifestText}
      ></textarea>{#if manifestResult}<div class="notice">
          {manifestResult}
        </div>{/if}
      <div class="actions">
        <button
          ...
          disabled={!manifestConnectorId || !manifestText}
