...
Legacy graph previews must target `graph_codemap_indexing`, include the historical preview-only reason `graph_runtime_indexing_disabled`, and never write graph records. Legacy semantic memory previews must target `semantic_memory_writes`, include the historical preview-only reason `semantic_vector_writes_disabled`, redact secret-like values, and never create embeddings, vectors, or durable semantic memory records. Current graph indexing, semantic memory, vector embedding/search, and provider-backed embedding runtimes are separate governed real executors.
