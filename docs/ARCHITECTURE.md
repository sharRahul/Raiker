...
| Phase 3 | Desktop/Web/Mobile/Plugin/Graph target platform architecture | Target platform architecture covers Desktop UI, Web UI, Dashboard, Apple mobile app, Android mobile app, plugin manager, semantic search, graph/codemap, REST API, and worktree isolation. Completed Phase 3 A-P scope provides the safe foundation/readiness layer, plus a launchable local web dashboard and authenticated single-user raiker-web API. Deferred runtime/app work includes Rich/native TUI, desktop, mobile, and IDE apps; runtime semantic/vector search, graph indexing/query execution, plugin execution, scheduled automation, and hosted/cloud runtime work. |
