...
> Current truth update (2026-07-23): **Hermes-informed local web experience is
...
> `docs/guide/webapp/HERMES_PHASE_0_EVIDENCE.md`: five disposable-workspace
> local-browser journeys were recorded, and the pass fixed owner visibility for
> sessionless connector-write approvals. Phase 1 browser viewport, keyboard, and
> visual-regression evidence remains outstanding.
...
- **Approval execution relay — generalized approve→execute (Workstream A / A1–A4, 2026-07-20)**: `implemented_verified`. **A1 (immutable intent):** every approval carries an immutable intent snapshot captured at creation (`SQLiteStore.insert_approval`): a canonical `action_payload_sha256` and a bounded `expires_at` TTL (default 24h). The relay re-verifies both at execution time — a payload that drifted since approval is refused (`approval_payload_tampered`, TOCTOU) and a past-TTL approval resolves to `expired` and never executes (`approval_expired`); `SQLiteStore.expire_approval` performs the pending→expired transition, and the metadata-only `ApprovalInbox.resolve` path plus `/api/approvals/{id}/resolve` enforce the same TTL (HTTP 409 `approval_expired`). **A2 (executor dispatch):** `ApprovalExecutionRelay` no longer only writes files — it re-routes the approved action through `RuntimeAuthority.route_action` as the approving human, so the target runs under its *own* capability gate state, decision mode, and PolicyEngine review at execution time (re-verified, not trusted from approval time); single-execution is enforced by an atomic `pending → executing → executed` state machine (`claim_approval_for_execution`/`finalize_approval_execution`/`release_approval_claim`), and a relay may never target another relay. **A3 (coverage):** dispatch is generic across every mapped capability with a registered executor (`edit_file`, `apply_patch`, `memory_write`, `memory_forget`, and Tier-2 `shell`/`process`/`web_fetch`/`network`); Tier-2 still requires its threat-ack-gated gate enabled, and critical-risk targets hit the human-confirmation floor and do not execute via the relay. **A4 (zero-trust hooks):** `raiker/runtime/authority/posture.py::capture_posture` builds a metadata-only posture snapshot (principal, session, interface, MFA enrolment) recorded on new `approval_executed`/`approval_execution_denied` events; a revoked approving session denies with `posture_degraded:session_revoked` before any claim or execution. Tests: `tests/test_approval_relay_general.py`, `tests/test_api_approvals.py::test_expired_approval_rejected`. Threat model: `docs/threat-models/approval-execution-relay.md`.
- **Checkpoint restore & rewind — pre-image capture (Workstream B / B1, 2026-07-20)**: `implemented_verified`. Every workspace-file mutation routed through `RuntimeAuthority.route_action` — the single executor-dispatch chokepoint shared by direct broker writes and the Workstream A approval relay — now records its **pre-image** so the mutation is reversible. `raiker/checkpoints/capture.py::CheckpointCaptureService.snapshot_pre_image` reads the target file's current bytes *before* the file-mutating executor runs; on executor success, `commit` writes those bytes into a content-addressed blob store under `.raiker/checkpoints/objects/<aa>/<sha256>` (deduplicated, atomic temp-file+rename) and inserts a metadata-only row into the new `checkpoint_capture_manifest` table (`SQLiteStore.insert_checkpoint_capture_entry` / `list_checkpoint_capture_entries`; migration `RAIKER-1201-checkpoint-capture-manifest`), then emits a metadata-only `checkpoint_captured` event. `capture_status` ∈ {`captured` (blob stored, restorable), `absent` (file did not exist → restore = delete), `oversize` (file over the 8 MiB cap, recorded honestly as not-restorable rather than silently dropped)}. Capture is **workspace-scoped** (paths escaping the workspace are not eligible), **size-capped**, **metadata-only** (no file content in the manifest or event log), **capture-on-success only**, and **best-effort/isolated** — a capture failure records `checkpoint_capture_failed` and never fails or blocks the underlying mutation. Only the two file-mutating Tier-1 capabilities (`file_write_execution`, `patch_apply_execution`) are capture-eligible; non-file mutations are not snapshot. B3 (fork), B4 (zero-trust escalation) remain planned. Tests: `tests/test_checkpoint_restore.py`. Threat model: `docs/threat-models/checkpoint-restore.md`.
- **Checkpoint restore & rewind — restore executor (Workstream B / B2, 2026-07-20)**: `implemented_verified`. `checkpoint_restore` is now an approval-required governed mutation routed as capability `checkpoint_restore_execution` (Tier 1, real executor → gate enabled by default) through the shared `route_action` chokepoint. `raiker/checkpoints/service.py::compute_restore_plan`/`plan_restore` upgrade the former `can_execute:False` stub into a **metadata-only dry-run diff**: restoring *to* a checkpoint rewinds every workspace file mutated after it to the pre-image of its first post-checkpoint B1 capture; each file entry carries op (`restore_content`/`delete`/`skip_oversize`), pre-image + current content-addresses and sizes, and a `changed` flag — never file content. `raiker/runtime/executors/tier1_checkpoint.py::CheckpointRestoreExecutor` re-derives the plan at execution time (trusts only `checkpoint_id`, never a caller-supplied file list), re-resolves each path against the workspace (skips escapes), re-verifies each pre-image blob against its content-address (skips missing/tampered rather than restoring corrupt content), and **captures its own pre-image of every file before overwriting/deleting it** (tagged with the restore's `action_id`) so a restore is itself reversible. `checkpoint_restore` is in the PolicyEngine approval-required set with default-`ask` decision mode, so an AI can only propose it (`needs_approval`); a human approval executes it via the Workstream A relay. CLI `/checkpoints restore <checkpoint_id>` renders the dry-run preview (execution stays on the governed propose→approve→relay path). B3 (fork), B4 (zero-trust escalation) remain planned. Tests: `tests/test_checkpoint_restore.py`, `tests/test_checkpoints.py`, `tests/test_phase_2_terminal_commands.py`. Threat model: `docs/threat-models/checkpoint-restore.md`.
