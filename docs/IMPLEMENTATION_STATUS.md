...
- **Approval execution relay — immutable intent (Workstream A / A1, 2026-07-19)**: `implemented_verified` — every approval now carries an immutable intent snapshot captured at creation (`SQLiteStore.insert_approval`): a canonical `action_payload_sha256` and a bounded `expires_at` TTL (default 24h). `ApprovalExecutionRelay` re-verifies both at execution time — a payload that drifted since approval is refused (`approval_payload_tampered`, TOCTOU defense) and a past-TTL approval resolves to `expired` and never executes (`approval_expired`). `SQLiteStore.expire_approval` performs the pending→expired transition (guarded on `status='pending'`), and the metadata-only `ApprovalInbox.resolve` path plus the `/api/approvals/{id}/resolve` endpoint enforce the same TTL (HTTP 409 `approval_expired`). Tests: `tests/test_approval_relay_general.py`, `tests/test_api_approvals.py::test_expired_approval_rejected`. Threat model: `docs/threat-models/approval-execution-relay.md`.
