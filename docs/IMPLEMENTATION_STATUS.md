...
| Uploaded document attachments (web-app task 3, text sub-slice): same governed store, text-format allowlist (plain text / markdown / csv) + 2 MB cap + UTF-8/NUL sniff (binary mislabelled as text fails closed), bounded extraction folded into context as an `untrusted_external` item (metadata-only load never carries bytes). PDF/office binaries out of scope for this slice | `implemented` | `raiker/runtime/attachments.py`, `raiker/api/routes_attachments.py`, `raiker/context/gatherer.py` | `tests/test_document_attachments.py` |
