...
| `/proposal <proposal_id> [--json] [--mark <proposed|acknowledged|deferred|rejected|superseded>]` | Phase 3 Slice A local CLI proposal lifecycle detail: shows one `ProposalLifecycleRecord` and `--mark` transitions status (metadata only). Metadata-only; proposal-only; no proposal execution, no auto-fix, no patch application, no file mutation, no staging/unstaging, no test execution, no shell/process/network execution, no approval execution. | `workspace:read` | Yes — metadata-only |
