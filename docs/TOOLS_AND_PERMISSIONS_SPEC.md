...
## Phase 3 status vocabulary reconciliation

Rows that mention Phase 3 must use one of: `implemented_verified`, `read_only_implemented_verified`, `metadata_only_implemented_verified`, `preview_only_implemented_verified`, `planning_only_implemented_verified`, `phase_scheduled_disabled`, `deferred_after_phase_3`, or `specified_not_implemented`. Completed Phase 3 slices A-P do not include notebook editing, semantic search runtime, symbol search runtime, graph query runtime, web search/fetch, LSP/MCP runtime operations, schedulers, plugin execution/skills/hooks/servers/monitors/tool adapters/panels/themes/config/dependency/reload/enable/disable/remove, memory writes/updates/forget/export, checkpoint restore/fork execution, transaction/snapshot execution, audit exports that expose unsafe data, or marketplace tools. Those items are `deferred_after_phase_3` or `specified_not_implemented` unless a row explicitly states a read-only/planning/metadata-only implementation.
