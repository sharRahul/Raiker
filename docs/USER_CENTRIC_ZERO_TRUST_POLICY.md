...
| ZT-3 | Identity and device posture (session validity, auth strength, MFA freshness, interface) MUST be captured on every governed action and re-verified between a recorded decision and its execution. | Posture snapshot | Planned — F1, A4 |
...
| ZT-7 | Critical-risk actions MUST notify the owner and resolve only by a live human's manual approve/reject with step-up verification. Their resting state is deny: silence, TTL expiry, session revocation, delegation, or any non-human resolution attempt resolves to deny. No decision mode, standing grant, scheduled routine, or subagent may resolve or pre-authorize a critical action. | Critical floor (`router.py`); production classification table + notify/decide lifecycle | Existing (floor); Planned — F6, F7 |
