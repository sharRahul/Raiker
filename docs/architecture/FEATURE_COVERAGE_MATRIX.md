...
| Hooks | implemented_policy_gated | Seventeen lifecycle events, all emitted and derived from call sites by test; `command`, bounded tool-free `prompt`, and Raiker `builtin` handlers; an owner off switch; per-rule "can decide" vs "observes only". `ConfigChange` intercepts authenticated settings writes with changed-key metadata only and cannot veto the kill switch. Seventeen of Claude Code's thirty-one events and two of its five handler types — see [reference compatibility](REFERENCE_PLATFORM_COMPATIBILITY.md#25-extensibility--hooks) |
...
| Channels | implemented_policy_gated | Pairing, enable switch, sender allowlist, inbound secret, 60/min per sender, signed outbound, owner-stored routing, tool-free side questions, interrupt/steer, and exact single-use owner approval response. Channel content is structurally data and cannot raise authority (FIXED-298). |
