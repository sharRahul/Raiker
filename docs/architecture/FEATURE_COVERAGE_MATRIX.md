...
| Hooks | implemented_policy_gated | Sixteen lifecycle events, all emitted, derived from the call sites by test; `command` and `builtin` handlers; an owner off switch; per-rule "can decide" vs "observes only". Sixteen of the thirty-one events, and one (`command`) of the five handler types the reference format documents — `builtin` is Raiker's own and is not one of the five — see [reference compatibility](REFERENCE_PLATFORM_COMPATIBILITY.md#25-extensibility--hooks) |
