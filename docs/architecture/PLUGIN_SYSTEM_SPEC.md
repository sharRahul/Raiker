...
The Raiker-native plugin component inventory and phase placement is tracked in [`docs/architecture/RAIKER_TOOL_AND_PLUGIN_CATALOG.md`](RAIKER_TOOL_AND_PLUGIN_CATALOG.md). Future builders must update that catalog when adding or changing plugin component types.
...
The strict manifest schema is defined in [`docs/architecture/PLUGIN_MANIFEST_SCHEMA.md`](PLUGIN_MANIFEST_SCHEMA.md). The example below is illustrative and must remain aligned with that schema.
