...
| MCP client over remote transport | Claude Code, Codex, Cowork | Extensions → MCP | Implemented | `http` transport with owner-added URL and optional token; monitored, not allowlist-blocked. Conformant since [FIXED-378](../plans/FIXED_ITEMS.md#fixed-378--raiker-spoke-the-current-mcp-revision-and-did-not-use-its-transport): `Accept` offers both framings (a conformant server may answer **406** without it), the `Mcp-Session-Id` is carried and released with `DELETE`, an expired session re-handshakes once, and a `401` with `WWW-Authenticate` is named as the OAuth requirement it is | No OAuth flow; a `text/event-stream` answer is read whole rather than streamed, with no resumability and no server-initiated messages | PARITY; **naming** what a server offers and Raiker does not use, on the server's own card, is a **YES — differentiator**: the compared clients degrade silently |
| Current MCP protocol revision | Claude Code, Codex, Cowork track the spec | Extensions → MCP | Implemented | `MCP_PROTOCOL_VERSION = "2026-07-28"` with three older revisions accepted (FIXED-274); the negotiated revision is stated on each server's card | Structured tool output, resource links, elicitation and `server/discover` are not used, and MCP Apps is carried as [ADD-24](../plans/TO_BE_ADDED.md). Each is **named on the card of a server that offers it** rather than implied by silence | PARITY |
