...
| Dictation that never leaves the device | None — every compared platform transcribes off the machine | Settings → Voice, Models → Local | Implemented | A loopback transcription runtime the owner points Raiker at, refused if the address is not on this machine; the disclosure under the microphone states which runtime is in use | — | **YES — differentiator** |
