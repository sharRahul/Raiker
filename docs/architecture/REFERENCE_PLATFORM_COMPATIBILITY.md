...
| Handler types | Claude Code: `command`, `http`, `mcp_tool`, `prompt`, `agent` | Platform-wide | Partial | `HANDLER_TYPES = {command, prompt, builtin}` | `http`, `mcp_tool` and `agent` remain refused (BUG-226). `builtin` is Raiker's own, not one of the five | PARITY; Raiker's advisory-only, owner-model prompt path is a **YES — improvement** |
