...
- [ ] F1 (ZT-3). **Posture snapshot on every governed action:** principal, session age, auth strength, MFA freshness, interface, decision-mode/grant used — appended to the existing event payloads (metadata-only). Powers A4/B4 escalation rules.
