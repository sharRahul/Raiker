...
- [x] B4. **Zero-trust hooks.** Restore counts as medium risk (auto-mode asks); a restore that would touch files modified by a *different* principal since the checkpoint escalates to high risk.
...
- [x] C2. **Capability subsetting.** A subagent's principal is created with an explicit capability subset ⊆ parent's; mutation proposals from subagents are routed into the *parent's* approval queue (subagents never approve, never relay).
- [x] C3. **Planner integration.** Classifier/planner can emit a `subagent_plan` for decomposable read/research tasks; gate flip (`subagents` → enabled) stays owner-only; decision mode default `ask`, with `auto` allowing read-only subagents unprompted (low risk).
