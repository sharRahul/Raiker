...
> [FIXED-559](FIXED_ITEMS.md#fixed-559--every-thread-opened-in-chat-including-the-ones-that-were-not),
> REM-SESSIONS as
> [FIXED-560](FIXED_ITEMS.md#fixed-560--the-evidence-inspector-was-a-second-place-to-resume-work),
> REM-MEM-01 as
> [FIXED-561](FIXED_ITEMS.md#fixed-561--seven-equally-prominent-actions-on-every-memory-card),
> REM-OBSERVE as
> [FIXED-562](FIXED_ITEMS.md#fixed-562--observability-opened-with-seven-tiles-saying-nothing-is-wrong),
> REM-EXT-01 as
> [FIXED-563](FIXED_ITEMS.md#fixed-563--extensions-asked-you-to-open-five-tabs-to-learn-what-you-had),
> REM-SKILL-01 as
> [FIXED-564](FIXED_ITEMS.md#fixed-564--three-ways-to-add-a-skill-all-open-at-once),
> REM-LIVE as
> [FIXED-565](FIXED_ITEMS.md#fixed-565--the-live-board-could-only-be-read-as-a-moving-picture)
> and REM-SET-SECURITY as
> [FIXED-566](FIXED_ITEMS.md#fixed-566--four-security-lifecycles-under-one-heading).
>
> **REM-THREAD-03 was a routing defect rather than a layout one**, which is why
> it is the only Medium among them: no session recorded which surface opened
> it, so every thread and every stored conversation was offered "open in chat" —
> including one whose repository, pending diffs and approvals are Build's. The
> composer's `surface` was already on the turn envelope and is now written down.
>
> Closing REM-SESSIONS left its own tail, filed rather than claimed: the
> conversation-library controls are still in the evidence inspector, because
...
> authority requirement and a negative bypass test, all CI-enforced — and found
> `image_generation` governing nothing while appearing to. The authority context
...
| ~~REM-HOME-03~~ **closed** — [FIXED-546](FIXED_ITEMS.md#fixed-546--four-pages-that-explained-themselves-before-they-did-anything) / P2 / S | Move repeated greeting/explanation and platform-health detail below the primary action. | Existing welcome/continue sections should answer “What next?” before teaching governance. | Keep one greeting, next action and concise exceptions; link to Guide/Observability. Verify first-run and returning-user mobile views; do not hide authentication or unresolved safety decisions. |
...
| ~~REM-EXT-01~~ **closed** — [FIXED-563](FIXED_ITEMS.md#fixed-563--extensions-asked-you-to-open-five-tabs-to-learn-what-you-had) | Move hook event catalogues, handler types and supply-chain explanation into Details/Guide. | ExtensionsView exposes useful operator reference alongside operational state. Installed, connected, permitted and usable need clear precedence. | Show installed inventory, actionable exceptions and Add first; reuse lifecycle controller across Connectors/MCP/Skills/Plugins. Preserve audit and signed manifest inspection. |
| ~~REM-SKILL-01~~ **closed** — [FIXED-564](FIXED_ITEMS.md#fixed-564--three-ways-to-add-a-skill-all-open-at-once) | Merge Upload/Import from link/Build one into a single Add skill entry with deliberate choices. | Multiple full forms compete before the user has chosen acquisition mode. | Shared staged preview, provenance/license check, permission diff and rollback receipt; skill learning produces reviewable versions. Test malicious metadata and upgrade requesting new grants. |
...
| ID / priority / effort | Remove, move or replace | Decision and explanation | Implementation and completion evidence |
| --- | --- | --- | --- |
| ~~REM-SET-GENERAL~~ **closed** — [FIXED-546](FIXED_ITEMS.md#fixed-546--four-pages-that-explained-themselves-before-they-did-anything) / P2 / S | Remove repeated setup teaching; move weather/location to optional personalization. | Language, timezone and startup should be short and explicit about UI versus model context. | Preserve IANA timezone and data-egress consent; test locale changes do not shift stored schedules. |
...
| ~~REM-SET-APPEARANCE~~ **closed** — [FIXED-546](FIXED_ITEMS.md#fixed-546--four-pages-that-explained-themselves-before-they-did-anything) / P2 / S | Move density/font tuning under Appearance details, retain Theme and reversible preview. | Preferences should not require understanding design tokens. | Preserve accessibility-safe options, zoom/reflow and system theme behavior; Cancel restores confirmed state. |
...
| ~~REM-OBSERVE~~ **closed** — [FIXED-562](FIXED_ITEMS.md#fixed-562--observability-opened-with-seven-tiles-saying-nothing-is-wrong) | Remove repeated healthy status panels from the default overview; preserve diagnostics as specialist views. | ObserveView asks several useful operational questions; show exceptions and recent changes first. | Canonical health records drive Home and Observe, with explicit stale/unknown. Do not hide failing security containment among optional telemetry errors. |
| ~~REM-SESSIONS~~ **closed** — [FIXED-560](FIXED_ITEMS.md#fixed-560--the-evidence-inspector-was-a-second-place-to-resume-work) | Merge ordinary resume actions into Threads; keep Sessions as an evidence inspector. | SessionsView exposes turn IDs and technical detail needed for audit, not an alternate everyday chat history. | Retain route aliases and turn anchors; tests open original events/checkpoints from Threads and approvals without duplication. |
