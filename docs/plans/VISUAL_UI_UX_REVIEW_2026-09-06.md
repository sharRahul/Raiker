...
| VIS2-13 badge/chip budget | Done — the budget is a shared rule now (`rowTokens.ts`): at most two badges per repeated row, attention first, and a fact in its ordinary state reads as metadata. Swept through Skills, Sessions and Approvals, and pinned by `visualRubric.test.ts` so a list cannot re-argue it locally ([FIXED-472](FIXED_ITEMS.md#fixed-472--every-row-wore-the-same-weight-as-the-row-that-needed-you)) |
| VIS2-14 theme-specific optical passes | Done — same palette, composed per ground: `--elevation-edge`, `--canvas-edge` and `--canvas-lift` differ between light and dark, because a shadow carries elevation on white and nothing on `#0B0D10`. Design's asset lost its card and carries its own hairline instead, obvious in both themes ([FIXED-481](FIXED_ITEMS.md#fixed-481--one-elevation-for-two-grounds-and-a-picture-inside-a-card)) |
| VIS2-15 4K/8K composition classes | Done — two `min-width` blocks in `app.css` move the workspace and operational canvases and nothing else; Build gives the extra room to the panes holding the object, not to the transcript. Prose measure, type scale, spacing and control sizes are pinned unchanged by `visualRubric.test.ts` and verified at 3840×2160 ([FIXED-481](FIXED_ITEMS.md#fixed-481--one-elevation-for-two-grounds-and-a-picture-inside-a-card)) |
