...
Provider policy, credential checks, egress policy, model binding, and all approval/runtime protections remain server-enforced. Tests cover the responsive Chat rail, no duplicate below-composer panel, selected/unselected model presentation, exact provider capability-driven effort choices, omission for unsupported models, rejection of invalid effort, and provider request propagation. Live testing covers Ollama, Anthropic, and OpenRouter model selection plus effort where the exact model advertises it. Credentials are process-scoped; no secret is stored in source, test data, logs, or documentation. Ollama cloud testing requires an available `OLLAMA_API_KEY`; a reachable local Ollama model may satisfy the Ollama slice instead.
