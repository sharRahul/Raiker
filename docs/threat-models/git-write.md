...
| A rename is one change, not half of one | A rename's *source* path is committed alongside its destination. Staging only the destination records the addition and leaves the old file's deletion behind — a half-recorded rename the owner was told was one change. The source half is already staged by `git mv`, so it is deliberately not re-`add`ed: it matches neither the working tree nor the index any more, and asking would fail the whole commit. |
