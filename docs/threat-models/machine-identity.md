...
| Use the owner's credential to gain owner authority | Storage resolves the delegated owner only for resource/control scope; RuntimeAuthority and executors retain the verified machine actor and model-controlled principal selectors cannot replace it | connector/provider, auto/skip posture, and broker tests |
...
| Approve after the original bearer expires | Approval storage snapshots the original public key/token IDs and issue/expiry times plus proposal hash, not bearer material; rotation cannot rewrite it and execution re-governs under current human authorization and gates | rotation snapshot, delayed approval, and proposal-integrity tests |
| Leak a machine principal after an abnormal exit | Every owning lifecycle closes in `finally` or a terminal helper for exceptions, abandoned streams, CLI actions, plugin relays, subagents, and resumes; approval suspension is the only deliberate non-terminal state | lifecycle cleanup tests |
| Mislabel a human/runtime event as agent-authored | Audit rows retain their literal `actor`; the turn identity is separate contextual metadata in the API and Activity UI | dashboard and Activity component tests |
