...
def handle_slash_command(command: str, *, workspace_root: str | Path = ".") -> str:
    ...
    if command == "/help":
        return (
            ...
            "Status: Phase 3 Slice B approval planning preview is implemented. Phase 3 is complete for safe foundation/readiness slices A-P; Phase 4 is blocked; runtime execution remains disabled. Current launchable UI is a local terminal client: a native Textual Rich TUI by default on a TTY, with a plain line-oriented shell fallback (RAIKER_TUI=plain, --prompt, or non-interactive stdin). Desktop/Web/Dashboard/Mobile/IDE/REST are contract-only or specified/deferred unless explicitly implemented. Phase 3 and Phase 4 commands are read-only, planning, preview, or metadata-only surfaces."
        )
