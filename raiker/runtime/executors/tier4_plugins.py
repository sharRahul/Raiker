...
class PluginRevocationExecutor:
    ...
    def execute(self, action: GovernedAction, principal: Principal) -> ExecutionResult:
        ...
        # BUG-221 — a revoked plugin's contributions are *deleted*, not annotated.
        # `HooksRegistry.load` reads files and has no store to consult, so leaving
        # the file behind would produce the one state this executor exists to
        # prevent: the page says revoked and the runtime still runs the rule. The
        # same deletion removes the plugin's skills directory, and the runtime
        # reconciles the skills store against that directory before every turn
        # advertises what is active — so the row cannot outlive the file either.
        # Reported in the artifacts either way, so a removal that did not happen
        ...
        return ExecutionResult(
            ...
            capability=self.capability,
            action_id=action.action_id,
            summary="Installed plugin revoked; its contributed hooks and skills are removed and brokered read-only execution is now denied for it.",
            artifacts={
                "record_id": record_id,
                "plugin_id": plugin_id,
