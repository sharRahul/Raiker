...
class RuntimeOrchestrator:
    ...
    async def _conversation_history(
        ...
        try:
            ...
            capacity = None
        fallback = conversation_messages(
            store, envelope.session_id, exclude_turn_id=envelope.turn_id,
            char_budget=history_char_budget(capacity),
        )
        principal_id = str(
            getattr(self.tool_broker, "principal_id", None) or envelope.user.id
        )
        compactions = ContextCompactionStore(store)
        active = compactions.active(principal_id, envelope.session_id)
        plan = ContextBudgetPlanner().plan(
            ...
            current_prompt=envelope.prompt.text,
    ...
    def _context_window_tokens(self, provider: str, model: str) -> int | None:
        ...
        try:
            ...
            owner_capacity = facts_store.owner_context_capacity(
                str(principal_id), provider, model
            )
            if owner_capacity is not None:
                return owner_capacity[0]
            facts = facts_store.provider_facts(str(principal_id), provider, model)
        except Exception:  # noqa: BLE001
            return None
        if facts is not None and facts.context_window_tokens is not None:
            return facts.context_window_tokens
        with contextlib.suppress(Exception):
            configured = self.model_router.registry.resolve(provider, model).raw.get(
    ...
    async def _aturn_events_inner(
        ...
        *,
        ...
        history = await self._conversation_history(envelope, messages)
        if history:
