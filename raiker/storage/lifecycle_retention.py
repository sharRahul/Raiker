...
ALLOWED_EXPIRY_RULES = {"none", "expire_after_30_days", "expire_after_90_days", "supersede_on_newer_preview", "manual_review_required"}
...
def create_retention_policy(*, lifecycle_target_type: str, retention_class: str, expiry_rule: str, cleanup_eligible: bool = False, legal_hold: bool = False, manual_hold: bool = False, reason_summary: str = "metadata-only retention planning", metadata: Mapping[str, Any] | None = None) -> StorageLifecycleRetentionPolicy:
    ...
    identity = {"lifecycle_target_type": lifecycle_target_type, "retention_class": retention_class, "expiry_rule": expiry_rule, "cleanup_eligible": bool(cleanup_eligible), "legal_hold": bool(legal_hold), "manual_hold": bool(manual_hold), "reason": reason, "metadata": safe_meta}
...
def create_cleanup_preview(*, linked_lifecycle_ids: Sequence[str], expired_candidate_count: int = 0, superseded_candidate_count: int = 0, summaries: Sequence[str] | None = None) -> StorageLifecycleCleanupPreview:
    links = _safe_list(linked_lifecycle_ids)
    redacted = [str(json_safe(s)) for s in _safe_list(summaries)]
    identity = {"linked_lifecycle_ids": links, "expired": expired_candidate_count, "superseded": superseded_candidate_count, "summaries": redacted}
    return StorageLifecycleCleanupPreview(stable_id("slcp_", identity), links, int(expired_candidate_count), int(superseded_candidate_count), redacted, False, False, False, False, False, False, False, False, False, False, False, True)
...
def create_approval_handoff(*, linked_lifecycle_ids: Sequence[str], target_capability: str, approval_state: str = "handoff_planned", source_preview_ids: Sequence[str] | None = None, source_audit_ids: Sequence[str] | None = None, source_rollback_plan_ids: Sequence[str] | None = None, summary: str = "approval handoff planned as metadata only") -> StorageLifecycleApprovalHandoff:
    ...
    identity = {"linked_lifecycle_ids": links, "source_preview_ids": previews, "source_audit_ids": audits, "source_rollback_plan_ids": rollbacks, "target_capability": target_capability, "approval_state": approval_state, "summary": safe_summary}
    return StorageLifecycleApprovalHandoff(stable_id("slah_", identity), links, previews, audits, rollbacks, target_capability, approval_state, False, False, safe_summary, True)
