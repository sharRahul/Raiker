...
def test_versioned_evaluation_corpus_enforces_scope_archive_and_supersession(tmp_path: Path) -> None:
    ...
    cases = seed_memory_eval_v1(store=store, workspace_root=tmp_path)
    report = evaluate_lexical_retrieval(
        store, corpus_version=MEMORY_EVAL_V1, cases=cases
    )
    ...
    assert {case.case_id for case in cases} == {
        "forgotten", "scope-and-archive", "sensitive-scope", "supersession", "valid-time"
    }
