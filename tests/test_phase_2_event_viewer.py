...
class TestEventViewer:
    ...
    def test_list_events_with_data(self, store: SQLiteStore, writer: EventLogWriter, viewer: EventViewer) -> None:
        sid = new_id("sess_")
        event = make_event(session_id=sid, turn_id=None, event_type="prompt_received", actor="test", payload={"text": "hello"})
        writer.append(event)
        events = viewer.list_events(session_id=sid, limit=10)
        assert len(events) == 1
        assert events[0]["event_type"] == "prompt_received"

    def test_list_events_filter_type(self, store: SQLiteStore, writer: EventLogWriter, viewer: EventViewer) -> None:
        sid = new_id("sess_")
        writer.append(make_event(session_id=sid, turn_id=None, event_type="prompt_received", actor="test"))
        writer.append(make_event(session_id=sid, turn_id=None, event_type="turn_closed", actor="test"))
        events = viewer.list_events(session_id=sid, event_type="turn_closed", limit=10)
        assert len(events) == 1
        assert events[0]["event_type"] == "turn_closed"

    def test_get_event_index(self, store: SQLiteStore, writer: EventLogWriter, viewer: EventViewer) -> None:
        sid = new_id("sess_")
    ...
    def test_read_event_payload(self, store: SQLiteStore, writer: EventLogWriter, viewer: EventViewer) -> None:
        sid = new_id("sess_")
        event = make_event(session_id=sid, turn_id=None, event_type="prompt_received", actor="test", payload={"text": "hello"})
        writer.append(event)
