...
def test_desktop_web_dashboard_return_equivalent_read_only_shapes(tmp_path) -> None:  # type: ignore[no-untyped-def]
    summaries = [inspect_workspace(client, workspace_root=tmp_path) for client in ("desktop", "web", "dashboard")]
...
def test_inspection_does_not_mutate_runtime_or_activate_gated_features(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    assert all(connector["default_state"] == "disabled" for connector in after["channel_connectors"] if connector["connector_id"] in {"channel.slack", "channel.email", "channel.teams", "channel.discord", "channel.signal"})
