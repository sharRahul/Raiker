...
<style>
  .environment-badge { display:inline-flex; align-items:center; gap:.38rem; min-height:28px; padding:.25rem .55rem; border:1px solid var(--border); border-radius:var(--r-pill); background:var(--surface); color:var(--text-2); font-size:var(--text-xs); text-decoration:none; white-space:nowrap; } /* the neutral-health rule — the healthy dot is neutral. This badge is on screen under every
