<script lang="ts">
  ...
  let { title, hint, current = "", quietHeader = false, connecting = false,
    ...
    current?: string;
    /** Work surfaces hide the route title and hint; see the markup (the Work-mode rule). */
    quietHeader?: boolean;
    connecting?: boolean;
  ...
  // Approvals came off the rail. A decision waiting on you is not a
...
<header class="topbar">
  ...
  <!-- On a work surface the mode switch beside this already says
  ...
  <!-- Raiker's product story is Assistant + Agent, and the shell said
       ...
       the three Work modes — and there are three of them. Drawing Chat and Build here while
...
<style>
  ...
  /* Navigation and attention on one side of the rule, where the
  ...
  /* A segmented control, not two links: the pair reads as one choice
     with a current value. Sentence case and no tracking (the uppercase rule). */
  ...
  /* The resting state is neutral. Colour arrives only when something
  ...
  .panel{position:absolute;right:0;top:calc(100% + 6px);z-index:var(--z-popover-panel);width:min(21rem,86vw);border:1px solid var(--border);border-radius:var(--r-md);background:var(--raised);box-shadow:var(--shadow-2);padding:var(--space-3)}
