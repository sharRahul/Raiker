...
describe("MemoryView observations (MEM-04)", () => {
  ...
  /*
   * REM-MEM-03 — the engine's controls moved to Settings → Memory engine, so
   * what this page owes an owner reading their own memories is the health of
   * recall and one link to repair it. The builder's own behaviour is covered
   * where it now lives, in `settings/MemoryEngine.test.ts`.
   */
  it("says how recall is matching, and links to where it is changed", async () => {
    stubFetch({
      "GET /api/memory": [],
      "GET /api/memory/settings": {
        incognito: false,
    ...
    expect(
      screen.getByRole("link", { name: /change the recall backend/i }),
    ).toHaveAttribute("href", "#/settings?tab=memory-engine");
    // The controls themselves are not here any more.
    expect(screen.queryByLabelText(/embedding model/i)).not.toBeInTheDocument();
    expect(screen.queryByLabelText(/recall backend/i)).not.toBeInTheDocument();
  });
